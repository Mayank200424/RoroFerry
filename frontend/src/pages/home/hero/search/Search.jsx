...
const Search = ({ onSearch }) => {
  ...
  useEffect(() => {
    ...
    if (savedSearchData && token) {
      ...
      // Call the onSearch function automatically
    ...
    >
      ...
      <div className="flex items-center gap-3 md:gap-5 text-center">
        <div className="flex flex-col items-center">
          ...
          <input type="text" value={from} onChange={(e) => setFrom(e.target.value)} list="from-locations"
        ...
        <div className="flex flex-col items-center">
          ...
          <input type="text" value={to} onChange={(e) => setTo(e.target.value)} list="to-locations"
            className="text-base md:text-2xl font-bold bg-transparent border border-neutral-300 p-2 rounded-lg text-center w-full" />
      ...
      <div className="flex flex-col items-center">
        ...
        <input type="date" className="bg-transparent text-sm md:text-xl border border-neutral-300 p-2 rounded-lg text-center w-full"
          value={departureDate} min={new Date().toISOString().split("T")[0]} onChange={(e) => setDepartureDate(e.target.value)} />
      </div>
      ...
      {tripType === "round" ? (
        <div className="flex flex-col items-center">
          ...
          <input type="date" className="bg-transparent text-sm md:text-xl border border-neutral-300 p-2 rounded-lg text-center w-full"
            value={returnDate} min={departureDate}
      ...
      {/* Search Button */}
      <button onClick={handleSearch} className="text-xl w-32 h-12 rounded-xl bg-blue-500 text-white relative overflow-hidden group z-10 hover:text-white duration-1000">
        SEARCH
      </button>
    </motion.div>
  );
